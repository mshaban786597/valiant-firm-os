/**
 * DEV-ONLY demo seed. Populates a full sample environment (clients, leads,
 * deals, tasks, reports, automations, AI logs, rank-and-rent, roadmap) for
 * local development and screenshots.
 *
 *   npm run db:seed:demo
 *
 * DO NOT run this against production. Production uses `npm run db:seed`
 * (prisma/seed.ts), which creates only system configuration. To remove demo
 * data created here, use `npm run data:clean-demo -- --confirm`.
 */
import {
  PrismaClient,
  LeadStatus,
